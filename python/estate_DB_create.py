...
# CSV 파일에서 데이터 불러오기
print("CSV 파일 로드 시작")
df_result = pd.read_csv('prp_dumy_data.csv', encoding='cp949')
print("CSV 파일 로드 완료")
...
with engine.connect() as conn:
    conn.execute(text("""
    CREATE TABLE IF NOT EXISTS prp_dumy_data (
...
df_selected.to_sql('prp_dumy_data', con=engine, if_exists='append', index=False)
