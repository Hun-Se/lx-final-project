...
# 데이터 불러오기
df = pd.read_sql("SELECT * FROM transaction_data1", con=engine)
...
# 필요한 피처 선택
X_2022 = df[['area', 'floor', 'construction_year', 'school_proximity', 'subway_proximity', 'total_floors', 'number_of_buildings']]
...
with engine.connect() as conn:
    result = conn.execute(text("SHOW COLUMNS FROM prp LIKE 'PRP_PREDICT_2022';"))
    column_exists = result.fetchone() is not None
    
    if not column_exists:
        conn.execute(text("ALTER TABLE prp ADD COLUMN PRP_PREDICT_2022 FLOAT;"))
print("PRP_PREDICT_2022 컬럼 추가 완료")
