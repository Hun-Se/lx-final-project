...
# CSV 파일에서 데이터 불러오기
print("CSV 파일 로드 시작")
df_result = pd.read_csv('real_estate_data1.csv', encoding='cp949')
print("CSV 파일 로드 완료")
...
with engine.connect() as conn:
    conn.execute(text("""
    CREATE TABLE IF NOT EXISTS real_estate_data1 (
...
df_selected.to_sql('real_estate_data1', con=engine, if_exists='append', index=False)
