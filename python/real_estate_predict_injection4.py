...
# 필요한 피처 선택
X_2023 = df[['area', 'floor', 'construction_year','deal_year', 'school_proximity', 'subway_proximity', 'total_floors', 'number_of_buildings']]
...
# Step 1: prp_dumy_data 테이블에 PRP_PREDICT_2022 컬럼 추가 (존재하지 않는 경우에만)
with engine.connect() as conn:
    result = conn.execute(text("SHOW COLUMNS FROM prp LIKE 'PRP_PREDICT_2025';"))
    column_exists = result.fetchone() is not None
    
    if not column_exists:
        conn.execute(text("ALTER TABLE prp ADD COLUMN PRP_PREDICT_2025 FLOAT;"))
print("PRP_PREDICT_2025 컬럼 추가 완료")
...
for index, row in dumy.iterrows():
    ...
    prp_predict_trade_year = row['prp_predict_trade_year']
    ...
    new_data = pd.DataFrame({
        ...
        'deal_year' : [prp_predict_trade_year],
    ...
    with engine.connect() as conn:
        result = conn.execute(
            text("""
            UPDATE prp
            SET PRP_PREDICT_2025 = :predicted_price
            WHERE prp_pk = :id
            """),
